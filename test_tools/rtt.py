...
unixtime = time.mktime(start_time.timetuple())
    ...
    try:
        while jlink.connected():
            ...
            if terminal_bytes:
                ...
                data = int(data.split(":")[1].split("\r")[0])
                queue.append(data)
...
def animate(i, xs, ys):
    ...
    if list(queue)!=[]:
        data = queue.popleft()
        ms = datetime.now()
        ms = time.mktime(ms.timetuple())
        xs.append(ms - unixtime)
        ys.append(data)
...
if __name__ == "__main__":
    sys.exit(main("NRF52832_XXAA", None if len(sys.argv) < 2 else int(sys.argv[2], 16)))